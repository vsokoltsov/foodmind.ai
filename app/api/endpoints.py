...
async def create_chat(
    ...
    async with SessionFactory() as session:
        conversation = await ConversationRepository(session).create(
            Conversation(user_id=payload.user_id, title=payload.title)
        )
...
async def _process_message(
    ...
    return ChatAnswerResponse(
        ...
        selected_agents=(dependencies.execution_state.selected_agents
                         if dependencies.execution_state else []),
        completed_steps=(dependencies.execution_state.completed_steps
                         if dependencies.execution_state else []),
...
async def get_chat(chat_id: UUID, request: Request, user_id: UUID = Query(...)) -> ChatResponse:
